-- Include all non-banned profiles; rank by total upvotes on approved listings, then listing count.
...
as $$
  ...
  left join public.extension_listings el
    on el.owner_id = p.id and el.status = 'approved'
  where coalesce(p.is_banned, false) = false
  group by p.id, p.username, p.full_name
  order by
    coalesce(sum(el.upvote_count), 0) desc,
    count(el.id) desc,
